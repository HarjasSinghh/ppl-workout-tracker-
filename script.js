...
// UNIFIED WORKOUT DATA STRUCTURE (FIX #1)
const workoutData = {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadCurrentUser();
    loadWorkoutProgress();
    renderHome(); // This will now correctly render the workout cards
});
...
function syncOrFetchData(event) {
    ...
    if (targetId.includes('Dashboard')) {
...
function updateSigninStatus(isSignedIn) {
    ...
    // Enable/disable other buttons based on sign-in status
...
function renderHome() {
    ...
    Object.entries(workoutData).forEach(([day, workout]) => {
        ...
        card.innerHTML = `<i class="fas fa-fire"></i><h3>${workout.name}</h3>`;
...
function saveWorkoutProgress() {
    ...
    // After saving, re-check sign-in status to update button disabled states
    const token = gapi.client.getToken();
    updateSigninStatus(token !== null);
}
