...
const workoutData = {
  1: {
    ...
    exercises: [
      ...
      },
      {
        ...
        sets: 3,
        reps: '15, 12, 10',
        alternatives: ['Rope pushdowns', 'skull crushers'],
      },
      {
  ...
  3: {
    ...
    exercises: [
      ...
      },
      {
        ...
        sets: 3,
        reps: '12, 10, 8',
        alternatives: ['Glute bridges', 'goblet squat', 'sumo squat'],
      },
      {
    ...
    exercises: [
      {
        ...
        sets: 2,
        reps: '12, 10',
        alternatives: ['Machine bench press', 'db flat bench press'],
      },
      {
        name: 'Cambered Bar Front Raise',
        sets: 3,
        reps: '15, 12, 10',
        alternatives: ['DB front raise', 'plate front raise'],
      },
      {
      ...
      },
      {
        ...
        sets: 2,
        reps: '20, 16',
        alternatives: ['DB front raise'],
      },
      {
        name: 'Close Grip Bench Press',
        sets: 2,
        reps: '15, 12',
        alternatives: ['Tricep dips'],
      },
      {
        name: 'Lateral Raises on Machine/Cable',
        sets: 2,
        reps: '20, 16',
        alternatives: ['Upright rows', 'db lateral raises'],
      },
    ],
  },
  5: {
    ...
    exercises: [
      {
        name: 'Close Grip Lat Pulldown w/ V Bar',
        sets: 3,
        reps: '15, 12, 10',
        alternatives: ['DB row', 'barbell row', 'pull ups'],
      },
      ...
      },
      {
        ...
        sets: 3,
        reps: '20, 16, 14',
        alternatives: ['Deadlift', 't-bar row'],
      },
      {
        name: 'Incline Curls',
        sets: 3,
  ...
  6: {
    ...
    exercises: [
      {
        name: 'Superset: Cable EZ Bar Curls / Tricep Pushdowns',
        sets: 4,
        reps: '15-15...',
        alternatives: [],
      },
      {
        name: 'Superset: Preacher Curls / Overhead Tricep Extension',
        sets: 3,
        reps: '12-12...',
        alternatives: [],
      },
      {
        name: 'Superset: Wide Grip Bar Curls / Rope Pushdowns',
        sets: 2,
        reps: '5p 10f - 10...',
        alternatives: [],
      },
      {
        name: 'Superset: Hammer Curls Drop Set / Single Arm Tricep',
        sets: 2,
        reps: '(15, 12, 10)...',
        alternatives: [],
      },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupEventListeners() {
    ...
    if (backToHomeBtn) backToHomeBtn.addEventListener('click', () => showPage('homeScreen'));

    document.querySelectorAll('[id^="authorizeBtn"]').forEach((btn) =>
      btn.addEventListener('click', handleAuthClick)
    );
    document.querySelectorAll('[id^="syncBtn"], #globalSyncBtn').forEach((btn) =>
      btn.addEventListener('click', syncOrFetchData)
    );
    const resetWorkoutBtn = document.getElementById('resetWorkoutBtn');
    if (resetWorkoutBtn) resetWorkoutBtn.addEventListener('click', resetCurrentWorkout);

    if (elements.workoutNotes) {
      elements.workoutNotes.addEventListener('input', saveNotes);
    }
    const analyzeBtn = document.getElementById('analyzeProgressBtn');
    if (analyzeBtn) analyzeBtn.addEventListener('click', analyzeProgressWithAI);

    const clearSheetBtn = document.getElementById('clearSheetBtn');
    if(clearSheetBtn) clearSheetBtn.addEventListener('click', clearGoogleSheet);
    ...
    if(chatToggleBtn) chatToggleBtn.addEventListener('click', () => elements.aiChatModal.classList.add('active'));
    const closeChatBtn = document.getElementById('closeChatBtn');
    if(closeChatBtn) closeChatBtn.addEventListener('click', () => elements.aiChatModal.classList.remove('active'));
  ...
  function loadWorkoutUI() {
    ...
    workout.exercises.forEach((exercise, exIndex) => {
      ...
      exercise.alternatives?.forEach(
        (alt) =>
          (optionsHtml += `<option value="${alt}">${alt}</option>`)
      );

      const selectedEx =
        progress.sets?.[exIndex]?.selectedExercise || exercise.name;
      const selectHtml = `<select class="exercise-select" data-ex="${exIndex}">${optionsHtml.replace(
        `value="${selectedEx}"`,
        `value="${selectedEx}" selected`
      )}</select>`;
      ...
      for (let setIndex = 0; setIndex < exercise.sets; setIndex++) {
        ...
        setsHtml += `<div class="set-row ${
          ...
          <input type="number" class="set-input" placeholder="kg" value="${
            p.weight || ''
    ...
    elements.exerciseListContainer
      .querySelectorAll('.set-checkbox, .set-input')
      .forEach((el) => el.addEventListener('change', handleSetChange));
    elements.exerciseListContainer
      .querySelectorAll('.exercise-select')
      .forEach((sel) => sel.addEventListener('change', handleExerciseChange));
  }
  ...
  function resetCurrentWorkout() {
    if (
      confirm(
        "Reset all entries for this session? This won't affect saved data in Google Sheets."
      ) && workoutProgress[currentDay]
    ) {
  ...
  async function syncWorkoutData() {
    ...
    try {
      ...
      const syncedDays = [
        ...new Set(
          dataToSync.map((row) =>
            Object.keys(workoutData).find((day) => workoutData[day].name === row[1])
          )
        ),
      ];
  ...
  function prepareDataForSheets() {
    ...
    for (const dayKey in workoutProgress) {
      ...
      for (const exIndex in progress.sets) {
        ...
        const exerciseName =
          progress.sets[exIndex].selectedExercise || defaultExercise.name;
  ...
  async function fetchDashboardData(showNotificationOnFail = true) {
    ...
    try {
      ...
      sheetData = (response.result.values || []).map((row) => {
        const dayName = row[1],
          dayKey = Object.keys(workoutData).find(
            (d) => workoutData[d].name === dayName
          );
        return {
  ...
  function populateExerciseFilter() {
    ...
    const exercises = [
      ...new Set(
        sheetData
          .filter((row) => row.user === currentUser && (bodyPart === 'all' || row.bodyPart === bodyPart))
  ...
  function renderDashboard() {
    ...
    let filteredData = sheetData.filter((row) => {
      ...
      const isDate =
        dateRange === 'all' || (new Date() - new Date(row.date)) / 86400000 <= parseInt(dateRange);
    ...
    const bestSet = filteredData.reduce(
    ...
    if (exercise === 'all' && bodyPart !== 'all') {
      ...
      chartData = Object.keys(groupedByDate).map((date) => ({
        ...
        e1RM:
          groupedByDate[date].reduce((a, b) => a + b, 0) / groupedByDate[date].length,
      }));
    } else {
  ...
  async function clearGoogleSheet() {
    ...
    if (
      !confirm(
        'Are you sure you want to delete ALL data from the Google Sheet? This action cannot be undone.'
      )
    )
      return;
  ...
  function selectUser(user) {
    ...
    loadCurrentUser();
    if (
      document.getElementById('dashboardScreen').classList.contains('active') &&
      isApiReady &&
